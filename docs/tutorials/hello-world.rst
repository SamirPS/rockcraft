...
This file permit to build a Rock with only the ``hello`` package slice inside 
using :ref:`chisel_explanation`. For more information about the ``parts`` section, 
go on :ref:`ref_parts`. The other fields of the file are `metadata
<https://canonical-rockcraft.readthedocs-hosted.com/en/latest/reference/
rockcraft.yaml/#format-specification>`_ which describe the ROCK.
